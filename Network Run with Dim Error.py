...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 25 21:04:24 2017
...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        for l in range(2, self.num_layers):
            ...
            # ^^^ ValueError: shapes (4,4) and (6,) not aligned: 4 (dim 1) != 6 (dim 0)
...
class DataGenerator(object):
    def __init__(self, ticker):
        ...
        self.rsi = talib.RSI(self.close)[15:]
    ...
    def create_factors(self):
        def factors():
            ...
            return rsi, mfi[15:], bbwidth[15:], bb_prime, roc[15:], willr[15:]

        return factors()

    def z_scores(self):
        rsi, mfi, bbwidth, bb_prime, roc, willr = self.create_factors()
        rsi_z, mfi_z, bbwidth_z, bb_prime_z, roc_z, willr_z = zscore(rsi), zscore(mfi), zscore(bbwidth), zscore(
            bb_prime), zscore(roc), zscore(willr)
        return rsi_z, mfi_z, bbwidth_z, bb_prime_z, roc_z, willr_z

    def generate_input_datasets(self):
        rsi_z, mfi_z, bbwidth_z, bb_prime_z, roc_z, willr_z = self.z_scores()
        x_data = np.array(list(zip(rsi_z, mfi_z, bbwidth_z, bb_prime_z, roc_z, willr_z)))
        return x_data
...
net = Network([6,4, 1])
net.SGD(list(zip(x_training, y_training)), 15, 1, .0001)
